...
// MSC - 040708 - remarked these as they are redundant and unused
/*
...
float4 materialAmbient : Specular
<
	string UIWidget = "Surface Ambient";
	string Space = "material";
> = {1.0f, 1.0f, 1.0f, 1.0f};
*/
...
uniform extern texture  gSpecMap;
...
float4 NormalMapAndEntityColorPS(float3 toEyeT    : TEXCOORD0,
                   float3 lightDirT : TEXCOORD1,
                   float2 tex0      : TEXCOORD2,
                   float fogLerpParam : TEXCOORD3) : COLOR
{
	...
	// Combine the color from lighting with the texture color.
	float3 f3color = ((lightAmbient + diffuse)*(texColor * EntityColorAdjust).rgb + spec);// + (texColor * illumColor);
                   ...
                   float fogLerpParam : TEXCOORD3) : COLOR
{
	...
	//float3 diffuse = s*(materialDiffuse*lightColor).rgb;
	//float3 ambient = materialAmbient*lightAmbient;
	//float3 spec = (t * tex2D(SpecMapS, tex0)) * lightSpecular.rgb;
	//float3 spec = (t * lightSpecular.rgb) * tex2D(SpecMapS, tex0);
	//float3 spec = (t * lightSpecular.rgb) * tex2D(SpecMapS, tex0);
	float4 specMap = tex2D(SpecMapS, tex0);
	float3 spec = t * lightSpecular.rgb * specMap;
	float3 diffuse = s * lightColor.rgb;
	//float3 ambient = materialAmbient*lightAmbient;
	...
	// Combine the color from lighting with the texture color.
	//float3 f3color = (((lightAmbient + diffuse)*(texColor * EntityColorAdjust).rgb + spec) * tex2D(SpecMapS, tex0)) + (texColor * illumColor)+(illumColor*RelColor);// * RelColor);
	//float3 f3color = ((lightAmbient + diffuse)*(texColor * EntityColorAdjust).rgb + spec) + (texColor * illumColor)+(illumColor*RelColor);// * RelColor);
	float3 f3color = ((lightAmbient + diffuse)*(texColor * EntityColorAdjust * specMap).rgb + spec) + (texColor * illumColor)+(illumColor*RelColor);// * RelColor);
	//float3 f3color = ((lightAmbient + diffuse)*(texColor * EntityColorAdjust).rgb + spec) + (texColor * illumColor)+(illumColor*RelColor);// * RelColor);
...
float4 PerPixelAndEntityColorPS(outPerPixelVS oIN) : COLOR
{
	float3 toEye = normalize(gEyePosW - oIN.posW);
	float3 r = reflect(lightDir, oIN.normalW);
	float t = pow(max(dot(r, toEye), 0.0f), shininess * 0.5F);
	...
	//float3 spec = t*(materialSpecular * lightColor).rgb;
	float3 spec = t * lightColor.rgb;
	float3 diffColor = tex2D( TexS, oIN.texCoordDiffuse )* EntityColorAdjust;
	float3 diffuseTexture = s * diffColor ;  // * (materialDiffuse * lightColor).rgb;
	...
	{
		f4Result = float4( ((lightAmbient + spec) * diffColor) + diffuseTexture , 1.0f);
	}
	return f4Result;
}
...
technique NormalMapWithEntityColor
{
    pass P0
    {
        // Specify the vertex and pixel shader associated with this pass.
        vertexShader = compile vs_1_1 NormalMapVS();
        pixelShader  = compile ps_2_0 NormalMapAndEntityColorPS();
    }
}
...
technique PerPixelWithEntityColor
{
    pass P0
    {
        // Specify the vertex and pixel shader associated with this pass.
        vertexShader = compile vs_1_1 PerPixelOnlyVS();
        pixelShader  = compile ps_2_0 PerPixelAndEntityColorPS();
    }
}
