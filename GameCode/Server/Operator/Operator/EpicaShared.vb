...
Public Enum eiTacticalAttrs As Short    '16 bit integer
    eFighterClass = 1               'In Space, Hull < MAX_FIGHTER_HULL, on ground, any unit that is aerial
    eCargoShipClass = 2             'Cargo Space > Hull / 3
    eCarrierClass = 4               'Hangar Space > Hull / 3, not available on the ground
    eTroopClass = 4                 'Unit Type = Infantry/Troopers, not available in space
    eEscortClass = 8                'In Space, Rating < BATTLESHIP_MIN_RATING and Hull > MAX_FIGHTER_HULL and Rating > MIN_ESCORT_RATING, on ground, Fast Assault Vehicles
    eBattleshipClass = 16           'In Space, Rating > BATTLESHIP_MIN_RATING, on ground, Main Battle Tanks
    eArmedUnit = 32                 'Any unit with at least 1 weapon
    eUnarmedUnit = 64               'Any unit with no weapons
    eTroopTransportClass = 128      'Land-Based Hangar bays
    eFacility = 256                 'A building, whether it is in space or on the ground
    eWeakestFirst = 512             'attack the weakest unit first or the strongest unit first
    eFurtherest = 1024              'attack the furthest unit first instead of the closest
End Enum
...
Public Enum eiBehaviorPatterns As Int32
    ...
    eTactics_UNUSED = 128                  'Not used, reserved for future use
...
Public Enum MineralCacheType As Byte
    ...
    ePickupable         'on surface or in space, does not require mining equipment to extract
End Enum
...
#End Region
    ...
    Public Const gl_CLIENT_VERSION As Int32 = 311
...
Public Class NetSock
	...
	Public Function GetRemoteIP() As String
		Return CType(moSocket.RemoteEndPoint, System.Net.IPEndPoint).Address.ToString
	End Function
End Class