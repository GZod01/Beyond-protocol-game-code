...
#End Region
    ...
    Public Const gb_IN_OPEN_BETA As Boolean = False

    Public Const gl_CLIENT_VERSION As Int32 = 311 '(311) FleetJump and other packet adds
	...
	Public Enum GlobalMessageCode As Short
        ...
        eGetRouteTemplates
        eEditRouteTemplate      'for add,update,delete
        eApplyRouteToEntities
        eBeginRouteForEntities
        eExceptionReport

        eSenateStatusReport
        eRequestMaxWpnRng

        eRequestColonyTransportPotentials
        eRequestTransportName
        eRequestTransportRouteDestList
        eSetTransportStatus
        eAddTransport
        eRequestTransports
        eRequestTransportDetails
        eDecommissionTransport

        eSubmitArenaConfig
        eArenaPlayerStatus
        eArenaUnitAssignment
        ...
        eGetGuildShareAssets
        eRequestUniversalAssets

		'===================================================
        eLastMsgCode                'MUST ALWAYS BE LAST!!!! AND THESE CODES MUST MATCH WITH THE REGION SERVER'S CODES!!!!
	End Enum
...
Public Class Epica_GUID
    ...
    Public InMyDomain As Boolean = False                'indicates that this GUID is controlled by me

    Public Sub DataChanged()
        mbStringReady = False
    End Sub

    Private Shared mlLastCycleSaveItem As Int32 = Int32.MinValue
    Public Sub QueueMeToSave()
        If mlLastCycleSaveItem = Int32.MinValue Then mlLastCycleSaveItem = glCurrentCycle
        mlLastCycleSaveItem += 1
        AddToQueue(mlLastCycleSaveItem, QueueItemType.eSaveObject, ObjectID, ObjTypeID, -1, -1, -1, -1, -1, -1)
    End Sub
...
Public Class NetSock
    ...
    Public Event onSocketClosed(ByVal Index As Int32)
    ...
    Public lAppType As MsgMonitor.eMM_AppType
    ...
    Public Sub SendData(ByVal Data() As Byte)
        Try
            ...
            If MsgSystem.mb_MONITOR_MSG_ACTIVITY = True Then
                goMsgSys.moMonitor.AddOutMsg(System.BitConverter.ToInt16(Data, 0), lAppType, Data.Length + 2, lSpecificID)
            End If

            If lAppType = MsgMonitor.eMM_AppType.ClientConnection AndAlso Data.Length > Int16.MaxValue Then
                Dim yTemp(Data.Length + 5) As Byte
                System.BitConverter.GetBytes(Int16.MinValue).CopyTo(yTemp, 0)
                System.BitConverter.GetBytes(CInt(Data.Length)).CopyTo(yTemp, 2)
                Data.CopyTo(yTemp, 6)
    ...
    Public Sub MakeReadyToReceive()
        ...
        Try
            ...
            moSocket.BeginReceive(oState.Buffer, 0, oState.BufferSize, 0, AddressOf DataArrivalCallback, oState)
        Catch
            If moSocket Is Nothing = False Then
                Try
                    moSocket.Disconnect(True)
                Catch
                End Try
                Try
                    moSocket.Shutdown(SocketShutdown.Both)
