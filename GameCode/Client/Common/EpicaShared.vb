...
Public Enum eiBehaviorPatterns As Integer
...
Public Class StateObject
    ...
    'Public Buffer(32766) As Byte
    Public BufferSize As Int32 = 100000
    Public Buffer(99999) As Byte
...
Public Class NetSock
    ...
    Public Sub Connect(ByVal sHostIP As String, ByVal lPortNumber As Int32)
        ...
        Catch ex As SocketException
            ...
            End If
            RaiseEvent onError(SocketIndex, ex.Message & ": " & ex.SocketErrorCode & ", I: " & sMsg & ", M: " & lMethod & ", L: " & sLoc)
        Catch oldEx As Exception
            Dim sMsg As String = ""
            If oldEx.InnerException Is Nothing = False Then
                sMsg = oldEx.InnerException.Message
            End If
            RaiseEvent onError(SocketIndex, oldEx.Message & ": " & sMsg & ", M: " & lMethod & ", L: " & sLoc)
        End Try
    End Sub

    Public Sub SendData(ByVal Data() As Byte)
        Try
            If Data.Length > Int16.MaxValue Then
    ...
    Private Sub DataArrivalCallback(ByVal ar As IAsyncResult)
        ...
        Try
            ...
            If lBytesRead = 0 Then
                ...
                Exit Sub
            End If
            ReDim oState.Buffer(oState.BufferSize - 1)
                ...
                Else
                    ...
                    Else
                        ...
                        If iOverrideLen < 1 Then
                            ...
                            lPos += 2
                            If iLen = Int16.MinValue Then
                                iLen = System.BitConverter.ToInt32(yData, lPos)
                                lPos += 4
                            End If
                        Else
                            iLen = iOverrideLen
