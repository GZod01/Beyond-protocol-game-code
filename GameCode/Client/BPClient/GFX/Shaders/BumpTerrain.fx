                     ...
                     float3 binormalL : BINORMAL0)
{
	...
	//MSC - 02/13/09 - these values are hardcoded to a certain planet, but they get the general idea of what I was attempting.
	//outVS.lightmag = 1.0f;
	//if (posL.y < 725)
	//{
	//	outVS.lightmag = ((1.0f - ((posL.y - 650) / 75.0f)) + 1.0f) * 3.0f;
	//}
                   ...
                   float2 tex1		: TEXCOORD4) : COLOR
{
	...
	//This does more detailed normal mapping on the terrain
	// this is only to effect the non-citycreep areas
	if (tex0.z > 0.125f)
	{
		normalT = tex3D(NormalMapS, float3(tex0.x * 2.0f, tex0.y * 2.0f, tex0.z));
		normalT = 2.0f * normalT - 1.0f;
		normalT = normalize(normalT);
		//s = s * (max(dot(lightVecT, normalT), 0.0f));		
		s = max(s, (max(dot(lightVecT, normalT), 0.0f)));
	}	
	...
	//MSC - 02/13/09 - could use some work, but real cool
	//f3color.r *= lightmag;
	//f3color.g *= lightmag * 0.25f;
                   ...
                   float2 tex1		: TEXCOORD4) : COLOR
{
	...
	// Get the texture color.
	float4 texColor = tex3D(TexS, tex0) * 2.5f;
...
float4 NormalMapFullNoFOW(float4 posH : POSITION0,
				   float3 toEyeT    : TEXCOORD0,
                   float3 lightDirT : TEXCOORD1,
                   float3 tex0      : TEXCOORD2,
                   float fogLerpParam : TEXCOORD3,
                   float2 tex1		: TEXCOORD4) : COLOR
{
	...
	// Get the texture color.
	float4 texColor = tex3D(TexS, tex0) * 2.5f;	//1.25f
	float3 f3color = ((lightAmbient + diffuse) * 0.3f) * texColor.rgb;
...
float4 NormalMapOnlyNoFOW(float4 posH : POSITION0,
				   float3 toEyeT    : TEXCOORD0,
                   float3 lightDirT : TEXCOORD1,
                   float3 tex0      : TEXCOORD2,
                   float fogLerpParam : TEXCOORD3,
...
technique NormalMapOnlyNoFOW
{
	pass P0
	{
		vertexShader = compile vs_1_1 NormalMapVS();
		pixelShader  = compile ps_2_0 NormalMapOnlyNoFOW();
	}
}

technique NormalMapFullNoFOW
{
	pass P0
	{
		vertexShader = compile vs_1_1 NormalMapVS();
		pixelShader  = compile ps_2_0 NormalMapFullNoFOW();
	}
}